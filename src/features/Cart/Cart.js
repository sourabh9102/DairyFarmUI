...
import cartProductSlice, { selectCartProduct, setAddToCart, removeItem } from './cartProductSlice';
...
export default function Cart() {
        ...
        const updatedProducts = products.map((product) =>
            product.id === productId
                ? {
                    ...
                    totalPrice: Math.max(1, product.quantity - 1) * product.price,
                }
                : product
        );
        setProducts(updatedProducts);
    ...
    const handleRemoveItem = (productId) => {
        ...
        setProducts(updatedProducts);
        localStorage.setItem('cart_product', JSON.stringify(updatedProducts));
