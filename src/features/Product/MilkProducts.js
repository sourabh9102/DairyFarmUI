...
export default function MilkProducts() {
    ...
    const handleProductOverview = (e, productId) => {
        console.log("productId:", productId)
        axios.post("http://localhost:5001/auth/productOverview", { id: productId })
            .then(res => {
                if (res.data.success) {
                    console.log("API res data:", res.data.products);
                }
            })
            .catch(err => {
                console.log("Error:", err);
            ...
            <div>
                ...
                <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                                            ...
                                            {renderSortedProducts.map((product) => (
                                                ...
                                                >
                                                    <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-md bg-gray-200 lg:aspect-none lg:h-44">
                                                        <Link onClick={(e) => handleProductOverview(e, product.id)}>
                                                    ...
                                                    <div className="mt-4 mx-3">
                                                        ...
                                                        <div className='flex mt-1 w-full justify-between mb-2'>
                                                            <p className="text-sm font-medium text-gray-900 translate-y-2">
                                                                ₹{' '}{product.price}
                                                            </p>
                                                            {showAddButton ? (
                                                                <div>
                                                                ...
                                                                <div>{handleIncrement(product.id)}</div>
                                                            )}
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    <div className='border-t border-gray-200 mb-5'>
                        <Stack
                            spacing={1}
                            style={{ justifyContent: 'flex-end', alignItems: 'flex-start', flexDirection: 'row', marginTop: '20px' }}
                        >
                            <Pagination count={10} />
                        </Stack>
                    </div>
