...
function NavListMenu(props) {
    ...
    const renderItems = navListMenuItems.map(
        ({ icon, title, description, to, category_id }, key) => (
            <Link to={to} key={key}>
                <MenuItem className="flex items-center gap-3 rounded-lg" onClick={() => props.handleCategory(category_id)}>
...
function NavList() {
    ...
    const handleCategory = (categoryId) => {
        axios.post('http://localhost:5001/auth/filter', { category_id: categoryId })
            .then(res => {
    ...
    return (
        <List className="mt-4 mb-6 p-0 lg:mt-0 lg:mb-0 lg:flex-row lg:p-1">
            ...
            >
                <ListItem className="flex items-center gap-2 py-2 pr-4">Our Process</ListItem>
            </Link>
            <Link
                as="Link"
                to="/about"
        ...
        <>
            <Navbar className="w-full mx-auto px-4 py-2 sticky top-0 z-10">
                <div className="flex items-center justify-between text-blue-gray-900">
                    ...
                    <Link to='/'><img src={logo} alt="Dairy Farm" className=" h-16" /></Link>
